...
I would expect the sample proportions to be somewhat similar but not a quite a match
...
samp2
```
Both samples are around the 80/20 ratio thus comparatively it makes sense they are close. I think as the size increaed the accuracy would increase.
...
15,000 elements and it is normally distributed in appearance
...
25 observations of people who dont think scientist work benefits them
...
All the observations represents that people dont the work scientists do benefits people like them. 
Sample size = 10,  mean of sampling distribution= 0.22, SE of sampling distribution =0.11 
Sample size = 50,  mean of sampling distribution = 0.2, SE of sampling distribution = 0.06
Sample size = 100, mean of sampling distribution = 0.2, SE of sampling distribution = 0.04

Surprisingly as the sample size increases we get a mean that stays pretty constant with our SE going down and with a more normal shape mean.
...
The graph is a bit skewed towards the right. Demonstrating that the better estiamte would be 80% of scientists
```{r}
...
Again another normal distribution, but it does look more normally distributed than 15 and is less of a spread.80% is the new guess
```{r}    
...
3 Has a smaller spread and a smaller spread would be preferential to making estimates closest to value
* * *
