...
I would expect it to be similar but not exactly the same. Since the sample is random and np and n*(1-p) are both greater than 10 the central limit theorem would apply to the samples.
...
3.  In the interpretation above, we used the phrase "95% confident". What does "95% confidence" mean?
    -We are 95% sure that the true population proportion is between the lower and upper bounds of the confidence interval.
...
5.  Each student should have gotten a slightly different confidence interval. What 
    ...
    mean? Why?
95% of the class would capture true population. 95% confidence means that if repeated 100 timeswe would expect to get at least 95 in the range.
...
6.  Given a sample size of 60, 1000 bootstrap samples for each interval, and 50 
  ...
  I would expect that up to 5% of the bootstrap simulated confidence intervals to not include the population proportion. Yield is 4% which is lower than 5%

```{r echo=FALSE, out.width = "100%"}

knitr::include_graphics("download.png", error = FALSE)
```
* * *
...
7.  Choose a different confidence level than 95%. Would you expect a confidence 
    ...
    88%. Therefore, I expect the range to be more narrow to allow less certainty
...
9.  Using the app, calculate 50 confidence intervals at the confidence level you chose 
    ...
    intervals?
    16% which is larger than the 12% range I was betwwen
```{r echo=FALSE, out.width = "100%"}

knitr::include_graphics("download2.png", error = FALSE)
```
...
10.  Lastly, try one more (different) confidence level. First, state how you expect the
    ...
    proportion.
    I expect the widht to be even smaller. It was 38%
```{r}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
...
knitr::include_graphics("download3.png", error = FALSE)
```
...
12.  Finally, given a sample size (say, 60), how does the width of the interval change 
 ...
 The width was the same for 100 or 1000 bootstraps. Doesn't seem to affect standard error
* * *
