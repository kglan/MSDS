...
We see in the data that the last of the histograms gives a more scaled view of the scenario as we are better able to see how strong the lower count departure delays are. However, the other views show more relativity regarding delays comparatively to the others.
...
3.  Describe the distribution of the **arrival** delays of these flights using a 
    ...
    The median arrival delay time is -11 mins while the average is -4.5mins
```{r}
...
4.  Calculate the median and interquartile range for `arr_delay`s of flights in
    ...
    DL and UA have the most variable arrival delay times, wiht both iqr being 22
```{r}
sfo_feb_flights %>%
  group_by(carrier) %>%
...
5.  Suppose you really dislike departure delays and you want to schedule 
    ...
    Mean- The pro to chosing the month with the lowest mean departure delay is that you will be having a greater chance of hitting that target departure delay time.The con to this idea is that the departure delay time you chose could land anywhere on the 1st percentile to the 100th percentile meanwhile your target is ther 50th percentile
    
    Median- Median allows you to select the most likely outcome of the departure time. This essentially means you are choosing the standard. However this does not take into account that the median is just a measure of frequency that could be up to random chance, so while you increase your odds at hitting that time it doesnt necessarily increase the accuracy. 
    
    In conclusion mean is for accuracy while median is for precision
...
6.  If you were selecting an airport simply based on on time departure percentage, which NYC airport would you choose to fly out of?
    You would chose LGA ass it has the best ratio of flights on time
...
ggplot(data=nycflightspd, aes(x=avg_speed, y=distance))+
  geom_point()
...
9.  Replicate the following plot. **Hint:** The data frame plotted only
    ...
    My though process here is that If we find the reverse times of arrival compared to the departure delat time then the flight arrives on time. For example if the flight was 5minutes early and was also 5minutes departing late that means you still made it on time.
...
dl_aa_ua_cut <- nycflights %>%
  filter(carrier == "AA" | carrier == "DL" | carrier == "UA")%>%
  filter(dep_delay == -(arr_delay))
ggplot(data = dl_aa_ua_cut, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point()
