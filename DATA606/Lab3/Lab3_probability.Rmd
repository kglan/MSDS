...
A streak of 1 means that there was 1 hit proceeding a miss. A streak of 0 means that there was 1 miss before the current miss.
...
```{r calc-streak-kobe}
kobestreak <- calc_streak(kobe_basket$shot)
kobe_streak <- data.frame(shot=1:76,
                          streak = kobestreak)
colnames(kobe_streak) <-  c("shot", "streak")
```

We can then take a look at the distribution of these streak lengths.

```{r plot-streak-kobe}
ggplot(data = kobe_streak, aes(x = streak)) +
  geom_bar()
```
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    Kobe's longest streak of 4 did not last very long and only happened once.
    From the data it shows that he missed more often than he streaked.
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    came up heads? Include the code for sampling the unfair coin in your response. 17 Heads 83tails
...
```{r set-seed}
set.seed(1234)                 # make sure to change the seed
```
...
sim_streak<- data.frame(shot=1:88,
                        streak=simstreak)

colnames(sim_streak) <-  c("shot", "streak")
...
The typical streak lenght is 0 and the longest streak of 5 happened twice.
```{r}
ggplot(data = sim_streak, aes(x = streak)) +
  geom_bar()
...
7.  If you were to run the simulation of the independent shooter a second time, 
  ...
  I would expect the distribution of shots to be more skewed to missing somewhat similar to the first. This is due to the idea that the shooting is still dependent and based on just probability we expect the shooter to miss more than make.
...
The shooting shown wihin Kobe's hooting demonstrates that he has demonstrations of a hot hand. This is due to him having a streak of 0 that is significantly less than the predictive model. This suggests that there is some change in probability of shooting depending on the shooter's make or missed shots.
* * *
