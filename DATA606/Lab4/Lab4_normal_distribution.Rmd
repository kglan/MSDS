...
```{r}
library(ggplot2)
mcdonalds %>%ggplot(aes(x=`cal_fat`))+
  geom_histogram(bins=30)

dairy_queen %>%ggplot(aes(x=`cal_fat`))+
  geom_histogram(bins = 30)
```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
    The plot seems to show the distribution to be left skewed. Thus not normal.
...
Upon viewing the data we see that not all of the points fall on the line.This demonstrates that the actual data is not normally distributed
...
4.  Does the normal probability plot for the calories from fat look similar to the plots 
    ...
    We notice that there is somewhat of a similarity between the plots, however 
    we also notice that the distribution significantly skews as the calories
    increase
...
5.  Using the same technique, determine whether or not the calories from McDonald's
    ...
    The distribution in calories from fat from mcdonalds is even less normally 
    distributed than that of airy queen
    
```{r}
mcmean <- mean(mcdonalds$cal_fat)
mcsd   <- sd(mcdonalds$cal_fat)
sim_norm_mc <- rnorm(n = nrow(mcdonalds), mean = mcmean, sd = mcsd)
...
The closer agreement went to the cholesterol level. Chick filA doesn't have a calorie from fat higher than 500
```{r}
#probability of 500 cal from fat at chickfilA

chicfilA<- fastfood%>%
  filter(restaurant == "Chick Fil-A")
View(chicfilA)

chicmean <- mean(mcdonalds$cal_fat)
chicsd   <- sd(mcdonalds$cal_fat)
1 - pnorm(q = 500, mean = chicmean, sd = chicsd)

#Emperical
chicfilA %>% 
  filter(cal_fat > 500) %>%
  summarise(percent = n() / nrow(chicfilA))


# Probability that cholesterol falls above 50

chomean <- mean(mcdonalds$cholesterol)
chosd   <- sd(mcdonalds$cholesterol)
1 - pnorm(q = 50, mean = chomean, sd = chosd)
...
This is due to the fact that there are various food items within the menus. Some require no sodium like a soft drink while others could be heavy in 
sodium, like salty food and fries
