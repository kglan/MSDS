...
2.  What type of plot would you use to display the relationship between the personal
    ...
    A standard plot of two variables would be sufficient as this factor is numerical and could be pit against another numerical varibale to recognize trends.
    No I would not be comfortable, we need to see the data through more lens to determine the progression forward.
```{r}
...
3.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    The points have a strong correlation coefficient of 0.796 which suggests they strongly influence each other.
...
4.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
  ...
  997 was the lowest. This was similar to the neighbors i built as i aimed to reach the  bottom left to top right each time
## The linear model
...
5.  Fit a new model that uses `pf_expression_control` to predict `hf_score`, or
    ...
    5.153687 + 0.349862 x pf_expression_control
    The slope tells us the steepness of the relationship, ie how direct their relationship is
```{r}
...
6.  If someone saw the least squares regression line and not the actual data, how
    ...
    I would predict it by finding the yhat for the regression where 6.7 meets the point.
    I don't think this is an overestimate as it falls within the range of the points, 
    it would be accurate dependent on the parameters of the regression
...
7.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    There is no apparent pattern. Thus illustrating the likelihood of a linearity
...
8.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    Yes it does, it seems a tiny bit skewed though
...
-   Choose another freedom variable and a variable you think would strongly
    ...
    pf_association and pf_rank
    It appears linear from a glance but no strong concentration of points along the regression
```{r}
ggplot(hfi, aes(x=pf_association, y=pf_rank)) + geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle("Human Freedom Index-Personal Freedom \nRank per Association") +
  xlab("Personal Freedom of Association") + ylab("Rank")
m3 <- lm(pf_rank ~ pf_association, data = hfi)
summary(m3)
```
...
-   How does this relationship compare to the relationship between 
    ...
    This prediction is less accurate as it has a R-squared of 0.5804 while 
    `pf_expression_control` and `pf_score` had a R-squared of 0.6342
...
-   What's one freedom relationship you were most surprised about and why? Display
    ...
    I was surprised by pf_expression_internet, pf_expression, you'd expect them 
    to walk hand in hand as the appeared to internet gave freedom of expression. 
    Initially i though it had no correlation but it had a 0.6277 R-squared and a
    successful q plot  but a weird looking fitted vs residual
```{r}
ggplot(hfi, aes(x=pf_expression_internet, y=pf_expression)) + geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  ggtitle("Human Freedom Index-Personal Freedom \nExpressionper Internet Expression ") +
  xlab("Personal Freedom of Internet Expression") + ylab("Personal Freedom Expression")

m5 <- lm(pf_expression ~ pf_expression_internet, data = hfi)
summary(m5)
