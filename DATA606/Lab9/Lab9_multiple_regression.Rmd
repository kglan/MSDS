...
1.  Is this an observational study or an experiment? The original research
    ...
    Is there a linear relationship between score received and beauty of professor?
    

2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    that tell you about how students rate courses? Is this what you expected to 
    see? Why, or why not?
    The distribution is skewed and that is what I expected as I dont think individuals
    really want to be harsh on professors unreasonably, so there wouldnt be a higher
...
3.  Excluding `score`, select two other variables and describe their relationship 
    ...
    ethnicity vs score
```{r}
hist(evals$cls_students, prob = TRUE, breaks = 16, main = "# Students per Class", xlab = "Age")
x <- seq(from = 20, to =95, by = 10)
curve(dnorm(x, mean = mean(evals$cls_students), sd = sd(evals$cls_students) ), add = TRUE, col = "red", lwd = 2)
...
4.  Replot the scatterplot, but this time use `geom_jitter` as your layer. What 
    was misleading about the initial scatterplot? The scatterplot seemed like 
    alphabet lettering by the overlapping points
...
5.  Let's see if the apparent trend in the plot is something more than
    ...
    y= 3.88034 + 0.06664 X score
    The R-squared value is terrible: 0.03502. NOT A GOOD PREDICTOR
...
6.  Use residual plots to evaluate whether the conditions of least squares
    ...
    The conditions seem reasonable besides the QQ plot tailing a bit, showing lack of 
    notmal distribution
```{r}
plot(m_bty, ask= FALSE)
```
...
7.  P-values and parameter estimates should only be trusted if the
    ...
    The data is still tailed in the QQ plot
```{r}
plot(m_bty_gen , ask = FALSE)

```

8.  Is `bty_avg` still a significant predictor of `score`? Has the addition
    of `gender` to the model changed the parameter estimate for `bty_avg`?
    Adding It has made the R-squared value double but it is still considerably 
    low as a predictor
...
9.  What is the equation of the line corresponding to those with color pictures? 
    ...
    3.74734+0.07416×bty_avg+0.17239×(x2)
    Male scores tend to have higher eval
...
10. Create a new model called `m_bty_rank` with `gender` removed and `rank` 
    ...
    It seems to handle the parameters well
```{r}
...
11. Which variable would you expect to have the highest p-value in this model? 
    ...
    I think the highest p value would be class level. This should be independent
...
12. Check your suspicions from the previous exercise. Include the model output
    in your response.
    My suspicion wasn't correct but it wasn't too far from the top.

13. Interpret the coefficient associated with the ethnicity variable.
Ethnicity multiplied by 1 in the equation
...
14. Drop the variable with the highest p-value and re-fit the model. Did the
    ...
    Yes the coefficients and significance saw a great change. This shows that
...
15. Using backward-selection and p-value as the selection criterion,
    ...
    removed ethnicity and got highest R-squared value so far
```{r}
...
16. Verify that the conditions for this model are reasonable using diagnostic 
    plots.
    The data is still skewed but not as bad as before 
```{r}
plot(m_full, ask = FALSE)
```
...
17. The original paper describes how these data were gathered by taking a
    ...
    This could encur more confusion as multiple teachers teach different courses
    and student can have different opinions

18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.
    A professor with ideal levels does not take into account minority and is heavily
    tailored to male teachers. 

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?
    No, this model is far too innacurate in prediction. The data is a bit skewed 
    thus not normal and the R-squared value is shockingly low.
