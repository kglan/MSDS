...
```{r}
df_H <- as.data.frame(unlist(hamtext), type = "ham" , stringsAsFactors = FALSE)
df_H$type <- "ham"
colnames(df_H)=c("text", "spam")

df_S <- as.data.frame(unlist(spamtext), type = "spam" , stringsAsFactors = FALSE)
df_S$type <- "spam"
...
corpus <- VCorpus(VectorSource(df_HS$text))
corpus <- corpus%>%
